// Shared article data for use across the application
export const articlesData = [
  {
    id: 1,
    title: "Summer Fashion Trends 2024: Bold Colors and Sustainable Fabrics",
    excerpt: "Discover the hottest fashion trends for summer 2024, featuring vibrant colors and eco-friendly materials.",
    category: "Fashion",
    image: "/images/articles/fashion/summer-trends-2024.jpg",
    slug: "summer-fashion-trends-2024",
    publishDate: "2024-04-15",
    author: "Emma Rodriguez"
  },
  {
    id: 2,
    title: "The Art of Mindful Living: Finding Balance in a Busy World",
    excerpt: "Learn practical techniques to incorporate mindfulness into your daily routine and improve your well-being.",
    category: "Lifestyle",
    image: "/images/articles/lifestyle/mindful-living.jpg",
    slug: "mindful-living-balance",
    publishDate: "2024-04-10",
    author: "David Chen"
  },
  {
    id: 3,
    title: "Clean Beauty Revolution: Ingredients to Look For and Avoid",
    excerpt: "Expert advice on creating a personalized skincare routine for healthy, glowing skin at any age.",
    category: "Beauty",
    image: "/images/articles/beauty/skincare-essentials.jpg",
    slug: "skincare-essentials-routine",
    publishDate: "2024-04-05",
    author: "Sophia Kim"
  },
  {
    id: 13,
    title: "Natural Skincare Routines for Every Skin Type",
    excerpt: "Discover personalized skincare regimens using natural ingredients for all skin concerns.",
    category: "Beauty",
    image: "/images/articles/beauty/natural-skincare-routines.jpg",
    slug: "natural-skincare-routines",
    publishDate: "2024-03-25",
    author: "Sophia Kim"
  },
  {
    id: 4,
    title: "The Evolution of Street Art: From Vandalism to Galleries",
    excerpt: "Exploring how street art has transformed from underground expression to mainstream cultural phenomenon.",
    category: "Culture",
    image: "/images/articles/culture/street-art.jpg",
    slug: "street-art-evolution",
    publishDate: "2024-03-28",
    author: "Marcus Johnson"
  },
  {
    id: 5,
    title: "Sustainable Fashion: Ethical Brands Leading the Way",
    excerpt: "Discover fashion brands that are prioritizing ethical production and environmental sustainability.",
    category: "Fashion",
    image: "/images/articles/fashion/sustainable-fashion-brands.jpg",
    slug: "sustainable-fashion-brands",
    publishDate: "2024-03-22",
    author: "Olivia Martinez"
  },
  {
    id: 6,
    title: "Home Organization Tips for a Clutter-Free Space",
    excerpt: "Transform your living space with these practical organization strategies and minimalist approaches.",
    category: "Lifestyle",
    image: "/images/articles/lifestyle/home-organization.jpg",
    slug: "home-organization-tips",
    publishDate: "2024-03-18",
    author: "James Wilson"
  },
  {
    id: 7,
    title: "Natural Makeup Looks for Everyday Elegance",
    excerpt: "Master the art of natural-looking makeup that enhances your features without looking overdone.",
    category: "Beauty",
    image: "/images/articles/beauty/natural-makeup.jpg",
    slug: "natural-makeup-looks",
    publishDate: "2024-03-15",
    author: "Aisha Patel"
  },
  {
    id: 8,
    title: "The Influence of Film on Fashion Through Decades",
    excerpt: "How iconic movies have shaped fashion trends and influenced designers from the 1950s to today.",
    category: "Fashion",
    image: "/images/articles/fashion/film-influence-fashion.jpg",
    slug: "film-fashion-influence",
    publishDate: "2024-03-12",
    author: "Emma Rodriguez"
  },
  {
    id: 9,
    title: "Digital Detox: Reclaiming Your Time and Attention",
    excerpt: "Practical strategies to reduce screen time and create healthier technology habits.",
    category: "Lifestyle",
    image: "/images/articles/lifestyle/digital-detox.jpg",
    slug: "digital-detox-guide",
    publishDate: "2024-03-10",
    author: "Michael Chang"
  },
  {
    id: 10,
    title: "The Science of Skincare: Understanding Ingredients",
    excerpt: "Demystifying common skincare ingredients and how they work to address different skin concerns.",
    category: "Beauty",
    image: "/images/articles/beauty/skincare-science.jpg",
    slug: "skincare-science-ingredients",
    publishDate: "2024-03-05",
    author: "Dr. Sarah Johnson"
  },
  {
    id: 11,
    title: "Global Music Trends Reshaping Pop Culture",
    excerpt: "How international sounds are influencing mainstream music and cultural expression.",
    category: "Culture",
    image: "/images/articles/culture/global-music.jpg",
    slug: "global-music-trends",
    publishDate: "2024-03-01",
    author: "Jamal Washington"
  },
  {
    id: 12,
    title: "Capsule Wardrobe: Simplify Your Style with Fewer Pieces",
    excerpt: "How to build a versatile wardrobe with fewer, high-quality pieces that mix and match effortlessly.",
    category: "Fashion",
    image: "/images/articles/fashion/capsule-wardrobe.jpg",
    slug: "capsule-wardrobe-guide",
    publishDate: "2024-02-25",
    author: "Sophia Kim"
  },
  // Add more articles for each category to reach 10 per category
  {
    id: 14,
    title: "Vintage Fashion Accessories: Timeless Pieces Worth Investing In",
    excerpt: "Discover the classic accessories that never go out of style and how to incorporate them into modern outfits.",
    category: "Fashion",
    image: "/images/articles/fashion/vintage-accessories.jpg",
    slug: "vintage-fashion-accessories",
    publishDate: "2024-02-20",
    author: "Emma Rodriguez"
  },
  {
    id: 15,
    title: "Sustainable Activewear: Performance Meets Environmental Responsibility",
    excerpt: "Explore eco-friendly workout gear that doesn't compromise on performance or style.",
    category: "Fashion",
    image: "/images/articles/fashion/sustainable-activewear.jpg",
    slug: "sustainable-activewear",
    publishDate: "2024-02-15",
    author: "Olivia Martinez"
  },
  {
    id: 16,
    title: "Color Psychology in Fashion: How Your Clothing Choices Affect Mood",
    excerpt: "Understanding the emotional impact of different colors and how to use them strategically in your wardrobe.",
    category: "Fashion",
    image: "/images/articles/fashion/color-psychology.jpg",
    slug: "color-psychology-fashion",
    publishDate: "2024-02-10",
    author: "David Chen"
  },
  {
    id: 17,
    title: "Fashion Week Highlights: Emerging Designers to Watch",
    excerpt: "Spotlight on the most promising new talent from international fashion weeks and their innovative collections.",
    category: "Fashion",
    image: "/images/articles/fashion/fashion-week.jpg",
    slug: "fashion-week-emerging-designers",
    publishDate: "2024-02-05",
    author: "Sophia Kim"
  },
  {
    id: 18,
    title: "Meditation Practices for Beginners: Starting Your Mindfulness Journey",
    excerpt: "Simple techniques to begin a meditation practice, even if you've never tried it before.",
    category: "Lifestyle",
    image: "/images/articles/lifestyle/meditation-beginners.jpg",
    slug: "meditation-beginners-guide",
    publishDate: "2024-02-01",
    author: "Michael Chang"
  },
  {
    id: 19,
    title: "Indoor Plants That Purify Air and Boost Mood",
    excerpt: "The best low-maintenance plants for improving air quality and creating a more positive home environment.",
    category: "Lifestyle",
    image: "/images/articles/lifestyle/indoor-plants.jpg",
    slug: "indoor-plants-benefits",
    publishDate: "2024-01-25",
    author: "James Wilson"
  },
  {
    id: 20,
    title: "Morning Routines of Successful People: Habits Worth Adopting",
    excerpt: "How leaders and innovators start their day, and the science behind why these habits work.",
    category: "Lifestyle",
    image: "/images/articles/lifestyle/morning-routines.jpg",
    slug: "successful-morning-routines",
    publishDate: "2024-01-20",
    author: "Olivia Martinez"
  },
  {
    id: 21,
    title: "Sustainable Living on a Budget: Small Changes with Big Impact",
    excerpt: "Affordable ways to reduce your environmental footprint without overhauling your entire lifestyle.",
    category: "Lifestyle",
    image: "/images/articles/lifestyle/sustainable-living.jpg",
    slug: "budget-sustainable-living",
    publishDate: "2024-01-15",
    author: "Marcus Johnson"
  },
  {
    id: 22,
    title: "Travel Journaling: Capturing Memories Beyond Photographs",
    excerpt: "Creative approaches to documenting your travels through writing, sketching, and collecting mementos.",
    category: "Lifestyle",
    image: "/images/articles/lifestyle/travel-journaling.jpg",
    slug: "travel-journaling-guide",
    publishDate: "2024-01-10",
    author: "Aisha Patel"
  },
  {
    id: 23,
    title: "Minimalist Skincare: Simplifying Your Routine for Better Results",
    excerpt: "Why less might be more when it comes to skincare, and how to build an effective minimalist regimen.",
    category: "Beauty",
    image: "/images/articles/beauty/minimalist-skincare.jpg",
    slug: "minimalist-skincare-routine",
    publishDate: "2024-01-05",
    author: "Dr. Sarah Johnson"
  },
  {
    id: 24,
    title: "Hair Care Myths Debunked by Experts",
    excerpt: "Professional stylists and trichologists set the record straight on common hair care misconceptions.",
    category: "Beauty",
    image: "/images/articles/beauty/hair-care-myths.jpg",
    slug: "hair-care-myths-debunked",
    publishDate: "2024-01-01",
    author: "Sophia Kim"
  },
  {
    id: 25,
    title: "Men's Grooming Essentials: Building a Simple but Effective Routine",
    excerpt: "A straightforward guide to skincare, haircare, and grooming products specifically for men.",
    category: "Beauty",
    image: "/images/articles/beauty/mens-grooming.jpg",
    slug: "mens-grooming-essentials",
    publishDate: "2023-12-25",
    author: "Marcus Johnson"
  },
  {
    id: 26,
    title: "Fragrance Layering: Creating Your Signature Scent",
    excerpt: "How to combine different perfumes and scented products for a unique personal fragrance.",
    category: "Beauty",
    image: "/images/articles/beauty/fragrance-layering.jpg",
    slug: "fragrance-layering-guide",
    publishDate: "2023-12-20",
    author: "Emma Rodriguez"
  },
  {
    id: 27,
    title: "The Renaissance of Vinyl Records in the Digital Age",
    excerpt: "Exploring the resurgence of vinyl collecting and why analog music continues to captivate new generations.",
    category: "Culture",
    image: "/images/articles/culture/vinyl-records.jpg",
    slug: "vinyl-records-renaissance",
    publishDate: "2023-12-15",
    author: "Jamal Washington"
  },
  {
    id: 28,
    title: "Independent Bookstores: Cultural Hubs in the Community",
    excerpt: "How local bookshops are evolving to become centers for connection, creativity, and cultural exchange.",
    category: "Culture",
    image: "/images/articles/culture/independent-bookstores.jpg",
    slug: "independent-bookstores-culture",
    publishDate: "2023-12-10",
    author: "David Chen"
  },
  {
    id: 29,
    title: "The Evolution of Dance: From Traditional to Contemporary Forms",
    excerpt: "Tracing the development of dance across cultures and how modern choreographers are pushing boundaries.",
    category: "Culture",
    image: "/images/articles/culture/dance-evolution.jpg",
    slug: "dance-evolution-contemporary",
    publishDate: "2023-12-05",
    author: "Aisha Patel"
  },
  {
    id: 30,
    title: "Food as Cultural Identity: Traditional Recipes and Their Stories",
    excerpt: "How culinary traditions reflect history, migration, and cultural values across different societies.",
    category: "Culture",
    image: "/images/articles/culture/food-cultural-identity.jpg",
    slug: "food-cultural-identity",
    publishDate: "2023-12-01",
    author: "Olivia Martinez"
  },
  {
    id: 31,
    title: "Digital Art Revolution: NFTs and the Changing Art Market",
    excerpt: "Understanding how blockchain technology is transforming art creation, ownership, and collection.",
    category: "Culture",
    image: "/images/articles/culture/digital-art.jpg",
    slug: "digital-art-nfts",
    publishDate: "2023-11-25",
    author: "Marcus Johnson"
  },
  {
    id: 32,
    title: "Cultural Festivals Around the World Worth Traveling For",
    excerpt: "Unique celebrations that offer authentic insights into different cultures and traditions globally.",
    category: "Culture",
    image: "/images/articles/culture/cultural-festivals.jpg",
    slug: "cultural-festivals-worldwide",
    publishDate: "2023-11-20",
    author: "Jamal Washington"
  },
  {
    id: 33,
    title: "Future Fashion Trends: What to Expect in 2025",
    excerpt: "A look ahead at the emerging fashion trends that will define style in the coming year.",
    category: "Fashion",
    image: "/images/articles/fashion/future-fashion-trends.jpg",
    slug: "future-fashion-trends-2025",
    publishDate: "2023-11-15",
    author: "Emma Rodriguez"
  },
  {
    id: 34,
    title: "Fashion History: How Styles Evolved Through Decades",
    excerpt: "Tracing the evolution of fashion from the early 20th century to today's diverse style landscape.",
    category: "Fashion",
    image: "/images/articles/fashion/fashion-history.jpg",
    slug: "fashion-history-evolution",
    publishDate: "2023-11-10",
    author: "David Chen"
  },
  {
    id: 35,
    title: "Wellness Retreats: Finding Peace in Nature",
    excerpt: "Discover the most rejuvenating wellness retreats around the world for mind, body, and soul restoration.",
    category: "Lifestyle",
    image: "/images/articles/lifestyle/wellness-retreats.jpg",
    slug: "wellness-retreats-nature",
    publishDate: "2023-11-05",
    author: "Michael Chang"
  },
  {
    id: 36,
    title: "Minimalist Living: Less Stuff, More Happiness",
    excerpt: "How embracing minimalism can lead to reduced stress, increased focus, and greater life satisfaction.",
    category: "Lifestyle",
    image: "/images/articles/lifestyle/minimalist-living.jpg",
    slug: "minimalist-living-happiness",
    publishDate: "2023-11-01",
    author: "James Wilson"
  },
  {
    id: 37,
    title: "Seasonal Beauty Transitions: Adapting Your Routine",
    excerpt: "How to modify your skincare and makeup routines as the seasons change for optimal results.",
    category: "Beauty",
    image: "/images/articles/beauty/seasonal-beauty.jpg",
    slug: "seasonal-beauty-transitions",
    publishDate: "2023-10-25",
    author: "Sophia Kim"
  },
  {
    id: 38,
    title: "Clean Beauty Brands Making a Difference",
    excerpt: "Highlighting innovative beauty companies committed to clean ingredients, sustainability, and ethical practices.",
    category: "Beauty",
    image: "/images/articles/beauty/clean-beauty.jpg",
    slug: "clean-beauty-brands",
    publishDate: "2023-10-20",
    author: "Aisha Patel"
  },
  {
    id: 39,
    title: "The Revival of Analog Hobbies in a Digital World",
    excerpt: "Exploring the resurgence of vinyl records, film photography, and other analog pastimes in our digital era.",
    category: "Culture",
    image: "/images/articles/culture/analog-revival.jpg",
    slug: "analog-hobbies-revival",
    publishDate: "2023-10-15",
    author: "David Chen"
  },
  {
    id: 40,
    title: "Virtual Museums: Experiencing Art in the Digital Age",
    excerpt: "How technology is transforming access to art and cultural artifacts through immersive online experiences.",
    category: "Culture",
    image: "/images/articles/culture/virtual-museums.jpg",
    slug: "virtual-museums-digital-art",
    publishDate: "2023-10-10",
    author: "Marcus Johnson"
  },
  {
    id: 41,
    title: "Cultural Appropriation vs. Appreciation in Fashion",
    excerpt: "Exploring the nuanced distinction between respectful cultural inspiration and harmful appropriation in the fashion industry.",
    category: "Culture",
    image: "/images/articles/culture/cultural-appreciation.jpg",
    slug: "cultural-appropriation-fashion",
    publishDate: "2023-10-20",
    author: "Sophia Kim"
  },
  {
    id: 100,
    title: "The Future of Sustainable Fashion",
    excerpt: "How innovative designers are reimagining the fashion industry with circular design, biotechnology, and digital solutions.",
    category: "Fashion",
    image: "/images/articles/fashion/future-sustainable-fashion.jpg",
    slug: "future-sustainable-fashion",
    publishDate: "2024-04-18",
    author: "James Wilson"
  }
];
